# WaveKit API Redesign — Full Changelog

Complete history of the WaveKit public API redesign, from the original 24-modifier surface down to the current consolidated version. Goal throughout: minimize modifiers a developer needs to touch while maximizing use cases covered, with `WaveView()` looking production-ready at zero config.
